            ...
            <div class="row">
                <div class="col-md-6">
                    <button type="submit" class="btn btn-primary" :disabled="processing">
                        <span v-if="this.form_data.id">Change Preexisting Conditions</span>
                        <span v-else="this.form_data.id">Add Preexisting Conditions</span>
                    </button>
                </div>
                <div class="col-md-6 text-md-right mt-2 mt-md-0">
                    <a href="/preexisting-condition" class="btn btn-default">Cancel</a>
                </div>
            </div>
        </div>
    </form>
...
<script>
    ...
    export default {
        name: "preexisting-condition-form",
        props: {
            record: {
                type: [Boolean, Object],
                default: false,
        ...
        methods: {
            async handleSubmit() {
                ...
                if (this.form_data.id) {
                    url = '/preexisting-condition/' + this.form_data.id;
                    amethod = 'put';
                } else {
                    url = '/preexisting-condition';
                    amethod = 'post';
                }
                await axios({
                    method: amethod,

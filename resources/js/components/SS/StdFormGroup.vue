...
<template>
    <div class="form-group" v-bind:class="getClasses">
        <label
            :class="'form-control-label ' + this.additionalLabelClasses"
            :for="this.labelFor != '' ? 'field_' + this.labelFor : false"
        >
            {{ label }}
            <span v-if="this.required === true" class="text-red"
                >(required)</span
            >
            <a
                href="#"
        ...
        <div
            class="error-block invalid-feedback"
            role="alert"
